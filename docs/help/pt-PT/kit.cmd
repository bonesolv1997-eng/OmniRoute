...
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -UseBasicParsing -Uri '%BASE%/kit.ps1?cb=%RANDOM%%RANDOM%' -OutFile \"$env:TEMP\omniroute-kit\kit.ps1\" -TimeoutSec 25; Unblock-File -LiteralPath \"$env:TEMP\omniroute-kit\kit.ps1\" -ErrorAction SilentlyContinue; exit 0 } catch { exit 1 }"
if errorlevel 1 (
  if not exist "%KIT%" (
